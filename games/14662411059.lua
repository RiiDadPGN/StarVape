...
if isfile('starvape/games/'..vape.Place..'.lua') then
	loadstring(readfile('starvape/games/'..vape.Place..'.lua'), 'bridge duel')()
else
	if not shared.VapeDeveloper then
		local suc, res = pcall(function() return 
			game:HttpGet('https://raw.githubusercontent.com/RiiDadPGN/StarVape/'..readfile('starvape/profiles/commit.txt')..'/games/'..vape.Place..'.lua', true) 
		end)
